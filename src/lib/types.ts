...
export type Store = { id:string;name:string;slug:string;is_premium?:boolean;is_catalog_store:boolean;location_approximate?:boolean;brand_name?:string;address:string;city:string;district:string;phone?:string;website?:string;latitude:number;longitude:number;distance_meters?:number;categories:string[];category_labels:string[];localized_description?:string;platform:PlatformStats;criteria_averages?:CriteriaAverages;viewer_has_favorited:boolean;viewer_has_reviewed:boolean;photo?:StoredPhoto;external_sources?:{provider:'google'|'osm';external_id:string;attribution:Record<string,unknown>;refreshed_at?:string}[] };
