...
export function I18nProvider({children,initialLocale='tr'}:{children:React.ReactNode;initialLocale?:Locale}){const [locale,setLocaleState]=useState<Locale>(initialLocale);const setLocale=(l:Locale)=>{setLocaleState(l);document.cookie=`bosagezme_locale=${l};path=/;max-age=31536000;samesite=lax`};const value=useMemo(()=>({locale,setLocale,t:(k:TranslationKey)=>dictionaries[locale][k]}),[locale]);return <Context.Provider value={value}>{children}</Context.Provider>}
