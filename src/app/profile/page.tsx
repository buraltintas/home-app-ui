...
export default function Page(){
  const {t,locale}=useI18n();const copy=accountCopy[locale];const [open,setOpen]=useState(false);const [signedIn,setSignedIn]=useState(false);const [checking,setChecking]=useState(true);const [deleting,setDeleting]=useState(false);
  useEffect(()=>{const controller=new AbortController();fetch('/api/proxy/me',{signal:controller.signal}).then(response=>setSignedIn(response.ok)).catch(()=>setSignedIn(false)).finally(()=>setChecking(false));return()=>controller.abort();},[]);
  const remove=async()=>{if(!window.confirm(`${copy.title}\n\n${deleteBody[locale]}`))return;setDeleting(true);try{const response=await fetch('/api/proxy/me',{method:'DELETE'});if(!response.ok)throw new Error();await fetch('/api/auth/logout',{method:'POST'});setSignedIn(false);}catch{window.alert(copy.failed);}finally{setDeleting(false);}};
  return <main className="empty-page"><p className="eyebrow">{t('profile')}</p><h1>{t('profileTitle')}</h1><p>{copy.body}</p>{!checking&&!signedIn&&<button className="button primary" onClick={()=>setOpen(true)}>{t('signIn')}</button>}<LocaleSwitcher/>{signedIn&&<section className="danger-zone"><h2>{copy.danger}</h2><p>{deleteBody[locale]}</p><button className="button secondary danger-button" disabled={deleting} onClick={()=>void remove()}>{copy.confirm}</button></section>}<AuthDialog open={open} onClose={()=>setOpen(false)}/></main>;
}
