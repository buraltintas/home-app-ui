...
export function SearchExperience() {
  ...
  return <main className="search-page"><header className="search-hero"><div className="search-title"><h1>{t('searchTitle')}</h1><span aria-hidden="true">↗</span></div><form className="search-form" onSubmit={submit} aria-busy={loading}><Search aria-hidden="true"/><textarea ref={field} rows={1} value={query} onChange={event=>setQuery(event.target.value)} onKeyDown={event=>{if(event.key==='Enter'){event.preventDefault();void runSearch();}}} placeholder={placeholder} aria-label={t('searchHint')} disabled={loading}/><button type="submit" disabled={loading}>{loading?t('loading'):t('searchAction')}</button></form><div className="location-control">{location?<><MapPin aria-hidden="true"/><span>{location.label}</span><button onClick={()=>setLocationOpen(true)} disabled={loading}>{t('change')}</button><button className="location-clear" aria-label={t('clearLocation')} onClick={()=>{setLocation(undefined);setData(undefined);}} disabled={loading}><X/></button></>:<button className="nearby-button" onClick={()=>setLocationOpen(true)} disabled={loading}><MapPin/>{t('nearby')}</button>}</div>{locationOpen&&<section className="location-sheet" aria-label={t('chooseLocation')}><div><h2>{t('locationTitle')}</h2><p>{t('locationBenefit')}</p></div><div className="location-actions"><button className="button primary" onClick={()=>void locateMe()} disabled={loading}><LocateFixed/>{t('useCurrentLocation')}</button><label><span>{t('chooseLocation')}</span><input value={manual} onChange={event=>setManual(event.target.value)} placeholder={t('locationHint')} disabled={loading}/></label><button className="button quiet" onClick={()=>setLocationOpen(false)}>{t('later')}</button></div>{manual.trim().length>=2&&<div className="location-results" aria-live="polite">{candidates.length===0?<p>{t('noLocations')}</p>:candidates.map(candidate=><button key={candidate.place_id} onClick={()=>choose(candidate)} disabled={loading}><strong>{candidate.name}</strong><span>{candidate.address}</span><small>{candidate.attributions.join(' · ')}</small></button>)}</div>}</section>}{!data&&!locationOpen&&!loading&&<div className="search-suggestions"><div><h2>{history.length?t('recent'):t('suggestions')}</h2>{Array.from(new Set(history.length?history.map(entry=>entry.raw_query):prompts)).map(phrase=><button key={phrase} onClick={()=>fill(phrase)}>{phrase} <ArrowRight/></button>)}</div><div><h2>{t('categories')}</h2><div className="category-links">{categories.map(category=><button onClick={()=>fill(category)} key={category}>{category}</button>)}</div></div></div>}{error&&<p className="form-error" role="alert">{error}</p>}</header>{loading&&<MascotLoader label={t('searchingStores')}/>}{!loading&&data?.guidance&&<section className="guidance-card"><h2>{data.guidance.message}</h2><div>{data.guidance.examples.map(example=><button key={example} onClick={()=>fill(example)}>{example}<ArrowRight/></button>)}</div></section>}{!loading&&data&&!data.guidance&&<section className="results-layout"><div className="result-list"><p className="result-count">{data.results.length} {t('results')}</p>{data.results.length===0?<div className="zero-state"><h2>{t('zeroTitle')}</h2><p>{t('zeroBody')}</p></div>:data.results.map(item=><Result item={item} key={item.search_result_impression_id} onSelect={()=>select(item)}/>)}</div></section>}</main>;
}
