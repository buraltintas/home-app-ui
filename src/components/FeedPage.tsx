...
export function FeedPage(){
  ...
  useEffect(()=>{let active=true;apiFetch('/api/proxy/feed?limit=20',{cache:'no-store',headers:{'X-Locale':locale},signal:AbortSignal.timeout(10000)}).then(async response=>{if(!response.ok)throw new Error();return response.json() as Promise<FeedResponse>;}).then(data=>{if(active){setError(false);setPosts(data.items);}}).catch(()=>{if(active){setPosts(null);setError(true);}});return()=>{active=false;};},[locale,request]);
