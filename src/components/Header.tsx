'use client';
import Image from 'next/image';import Link from 'next/link';import {usePathname,useRouter} from 'next/navigation';import {useEffect,useState} from 'react';import {useI18n} from '@/i18n/I18nProvider';import {localePath,stripLocale,tagline} from '@/lib/site';import {apiFetch,SESSION_REFRESHED} from '@/lib/api-client';import {HeaderLocale} from './HeaderLocale';
import {NavFavorites,NavHome,NavProfile,NavSearch} from './NavIcons';import type {Me} from '@/lib/types';
...
export function Header(){
  ...
  return <header className="site-header"><div className="nav-wrap"><Link href={localePath(locale,'/')} className="brand-link" aria-label={t('wordmark')}><span className="brand-mark"><Image src="/brand/brand-mark.png" width={104} height={104} priority alt=""/></span><span className="brand-lockup"><span className="brand-name">{t('wordmark')}</span><span className="brand-tagline" lang="tr">{tagline}</span></span></Link><nav aria-label={t('primaryNavigation')}>{links.map(([href,Icon,label])=>{const active=href==='/'?here===href:here.startsWith(href);return <Link key={href} href={localePath(locale,href)} className={active?'is-active':undefined} aria-current={active?'page':undefined}><Icon/><span>{t(label)}</span></Link>})}<Link href={localePath(locale,"/profile")} className={profileActive?'is-active':undefined} aria-current={profileActive?'page':undefined}>{me===undefined?<span className="nav-avatar is-loading" aria-hidden="true"/>:me?<span className="nav-avatar" aria-hidden="true">{(me.display_name||me.email).slice(0,1).toLocaleUpperCase(locale)}</span>:<NavProfile/>}<span>{t('profile')}</span></Link></nav><HeaderLocale/></div></header>;
}
