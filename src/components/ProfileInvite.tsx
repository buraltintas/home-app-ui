...
const copy:Record<Locale,{title:string;body:string;share:string;shared:string;failed:string}>={
  tr:{title:'Topluluğu güçlendir',body:'Gerçek ev ve yaşam mağazalarını\nbirlikte keşfedin',share:'Boşa Gezme!’yi Paylaş',shared:'Bağlantı kopyalandı',failed:'Bağlantı kopyalanamadı'},
  en:{title:'Strengthen the community',body:'Discover real home and living stores\ntogether',share:'Share the website',shared:'Link copied',failed:'Could not copy the link'},
  de:{title:'Die Community stärken',body:'Entdeckt gemeinsam echte\nWohn- und Haushaltsgeschäfte',share:'Website teilen',shared:'Link kopiert',failed:'Link konnte nicht kopiert werden'},
  ru:{title:'Укрепите сообщество',body:'Открывайте настоящие магазины\nдля дома вместе',share:'Поделиться сайтом',shared:'Ссылка скопирована',failed:'Не удалось скопировать ссылку'},
};
...
export function ProfileInvite({locale}:{locale:Locale}){
  ...
  return <section className="profile-invite" aria-labelledby="profile-invite-title">
    <div className="profile-invite-copy">
      ...
      <div className="profile-invite-head">
        <svg className="profile-invite-mark" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          <circle cx="12" cy="13" r="2.6" fill="currentColor"/><path d="M8 20.5a4.2 4.2 0 0 1 8 0" fill="currentColor"/>
          <circle cx="5" cy="15" r="2.1" fill="currentColor"/><path d="M1.8 20.8a3.6 3.6 0 0 1 4.6-2.6" fill="currentColor"/>
          <circle cx="19" cy="15" r="2.1" fill="currentColor"/><path d="M22.2 20.8a3.6 3.6 0 0 0-4.6-2.6" fill="currentColor"/>
    ...
    <Image className="profile-invite-phone" src="/illustrations/share-phone.webp" width={476} height={625} alt="" aria-hidden="true"/>
