...
export function TimedNudge({kind,requireReviewFlag=false}:{kind:Kind;requireReviewFlag?:boolean}){
  ...
  useEffect(()=>{
    ...
    const dismiss=()=>{dismissed=true;window.clearTimeout(timer);setLeaving(true);exitTimer=window.setTimeout(()=>setVisible(false),220);};
    window.addEventListener('scroll',dismiss,{passive:true,once:true});
    return()=>{window.clearTimeout(timer);if(exitTimer!==undefined)window.clearTimeout(exitTimer);window.removeEventListener('scroll',dismiss);};
